...
def create_models_router(auth_dependency: Callable) -> APIRouter:
    ...
    @router.get(
        "", response_model=List[FinetunedModelRead], status_code=status.HTTP_200_OK
    )
    def get_models(base_fm: BaseFM = None, db: Session = Depends(get_db)):
        """Get all finetuned models."""
        try:
            models = db.query(FinetunedModel)
            if base_fm:
                models = models.filter(FinetunedModel.base_fm == base_fm)
            models = models.all()
            return models
    ...
    def create_model(
        ...
        if "admin" not in user_groups:
            raise HTTPException(
                ...
                detail=f"You do not have permission to create {model.name} model",
            )
        try:
    ...
    @router.get(
        "/{model_id}", response_model=FinetunedModelRead, status_code=status.HTTP_200_OK
    )
    def get_model(model_id: str, db: Session = Depends(get_db)):
        """Get a specific model by ID."""
        try:
            model = (
                db.query(FinetunedModel).filter(FinetunedModel.id == model_id).first()
            )
            if not model:
    ...
    def get_model_inferences(
        ...
        try:
            inferences = (
                ...
                .filter(FinetunedModel.id == model_id)
                .all()
            )
            return inferences
        except Exception as e:
    ...
    @router.get(
        "/{model_id}/preloaded_events",
        response_model=List[PreloadedEventRead],
        status_code=status.HTTP_200_OK,
    )
    def get_model_preloaded_events(
        model_id: str,
        _: Dict[str, Any] = Depends(auth_dependency),
        db: Session = Depends(get_db),
    ):
    ...
    def delete_model(
        ...
        try:
            ...
            if "admin" not in user_groups:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail=f"You do not have permission to delete {model.name} model",
                )
            db.delete(model)
            db.commit()
