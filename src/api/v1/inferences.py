        ...
        async with _temporal_client_lock:
            if _temporal_client is None:
                _temporal_client = await Client.connect(
                    TEMPORAL_SERVER_URL, namespace=TEMPORAL_NAMESPACE
                )
...
def create_inference_router(auth_dependency: Callable) -> APIRouter:
    ...
    def get_inferences(
        ...
        try:
            ...
            query = db.query(Inference).filter(Inference.user_email == user_email)

            if base_fm:
                query = query.filter(Inference.finetuned_models.any(base_fm=base_fm))
    ...
    @router.get(
        "/{inference_id}", response_model=InferenceRead, status_code=status.HTTP_200_OK
    )
    ...
    @router.get(
        "/{inference_id}/preloaded_events",
        response_model=List[PreloadedEventRead],
        status_code=status.HTTP_200_OK,
    )
    def get_inference_preloaded_events(
        ...
        try:
            events = (
                ...
                .filter(PreloadedEvent.inference_id == inference_id)
                .all()
            )
            return events
        except Exception:
    ...
    def create_inference(
        ...
        inference_name = (
            inference.name
            if inference.name
            else datetime.now(timezone.utc).strftime("inference_%Y%m%d_%H%M%S")
        )
        finetuned_models = (
            db.query(FinetunedModel)
            .filter(FinetunedModel.id.in_(inference.finetuned_model_ids))
            .all()
        )
        if not finetuned_models or len(finetuned_models) != len(
            inference.finetuned_model_ids
        ):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="One or more finetuned models not found",
            )
        ...
        if finetuned_models[0].name == "Surya":
            ...
            url = f"http://{model_id.replace('_', '-')}-service-lb:{port}/api/v1/invocations"
            t = time.time()
            ...
            for finetuned_model in finetuned_models:
                ...
                if model_id not in user_groups:
                    raise HTTPException(
                        ...
                        detail=f"Key provided is not authorized to run inference on {model_id}",
                    )
            results = {}
            for finetuned_model in finetuned_models:
                ...
                for date, merged_file in prepared_data.items():
                    ...
                    response = requests.post(
                        ...
                        json={
                            ...
                            "model_id": model_id,
                            "qa_flags": finetuned_model.data_config.get(
                    ...
                    print(
                        f"Inference service Took to respond {time.time() - t:.2f} seconds"
                    )
                    results[model_id] = results.get(model_id, {})
                    results[model_id][date] = results[model_id].get(date, {})
                    results[model_id][date] = response.json()[model_id]

                infered_results = results[model_id][date] or EMPTY_RESPONSE
                ...
                result_entry["postprocess_links"] = infered_results.get(
                    "postprocess_links", {}
                )
    ...
    @router.post(
        "/queue", status_code=status.HTTP_201_CREATED, response_model=InferenceRead
    )
    async def create_queue_inference(
        inference: InferenceUpdate,
        claims: Dict[str, Any] = Depends(auth_dependency),
        db: Session = Depends(get_db),
    ):
        """Create a new queued inference that offloads work to Temporal."""
        t0 = time.time()
        user_groups = claims.get("groups") or claims.get("cognito:groups", [])
        user_email = claims.get("email")
        inference_name = (
        ...
        if finetuned_models[0].name == "Surya":
            # Preserve existing Surya behavior (inline HTTP call)
            finetuned_model = finetuned_models[0]
            model_id = str(finetuned_model.source_details.get("model_id"))
            port = finetuned_model.source_details["port"]
            url = f"http://{model_id.replace('_', '-')}-service-lb:{port}/api/v1/invocations"
            ...
            # For non-Surya models, validate authorization up front
            for finetuned_model in finetuned_models:
                model_id = str(finetuned_model.source_details.get("model_id"))
                if model_id not in user_groups:
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
                        detail=(
                            f"Key provided is not authorized to run inference on {model_id}"
                        ),
                    )
        ...
        inference_details["finetuned_models"] = finetuned_models
        inference_details["user_email"] = user_email
        inference_details.pop("status", None)
        del inference_details["finetuned_model_ids"]
        inference_orm = Inference(**inference_details)
        inference_orm.status = InferenceStatus.queued
        db.add(inference_orm)
        db.commit()
        db.refresh(inference_orm)

        # For non-Surya models, start the orchestrator workflow (returns immediately)
        if finetuned_models[0].name != "Surya":
            client = await get_temporal_client()
            workflow_id = f"orchestrator-{inference_orm.id}"

            await client.start_workflow(
                "InferenceOrchestrator",
                {
                    "inference_id": str(inference_orm.id),
                    "model_configs": build_model_configs(finetuned_models),
            ...
            logger.info(
                "Started InferenceOrchestrator workflow %s for inference %s",
                workflow_id,
                inference_orm.id,
            )

        logger.info("Queue endpoint took %.2f seconds", time.time() - t0)
        return inference_orm
