...
from fastapi import FastAPI, APIRouter, Request, Depends, status, HTTPException
...
# To generate a new token you can use the following command:
...
def get_user_groups_from_cognito(username: str) -> list[str]:
    ...
    try:
        ...
        logger.info(
            f"Fetching groups for user: {username} from pool: {COGNITO_USER_POOL_ID}"
        )
        ...
        logger.error(
            f"Error fetching user groups from Cognito: {type(e).__name__}: {str(e)}"
        )
        return []


async def get_user_groups_from_cognito_async(username: str) -> list[str]:
    """Async wrapper for getting user groups from Cognito."""
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(_executor, get_user_groups_from_cognito, username)
...
async def verify_cognito_token(
    ...
    try:
        result = await asyncio.get_running_loop().run_in_executor(_executor, _verify)
        logger.info(
            f"Successfully authenticated user '{result.get('username')}' via Cognito token"
        )
        return result
    except Exception as e:
...
async def general_access_dependency(
    ...
    if custom_token_payload:
        logger.info(
            f"Authenticating via custom JWT for user '{custom_token_payload.get('sub')}'."
        )
        return custom_token_payload
    if cognito_token_payload:
        logger.info(
            f"Authenticating via Cognito token for user '{cognito_token_payload.get('username')}'."
        )
        return cognito_token_payload
    logger.info(
        "No valid JWT or Cognito token found. Falling back to Header authentication."
    )
    try:
...
@v1_router.post("/create-token", tags=["Authentication"])
async def create_token(
    ...
    if not allowed_groups:
        raise HTTPException(
            status_code=403,
            detail="The user does not belong to any of the groups required to access the API.",
        )
...
def _authenticate_with_cognito_sync(username: str, password: str) -> dict:
    """Synchronous Cognito authentication - runs in thread pool"""
    try:
        auth_params = {
            "USERNAME": username,
            "PASSWORD": password,
...
async def authenticate_with_cognito(username: str, password: str) -> dict:
    ...
    return await asyncio.get_running_loop().run_in_executor(
        _executor, _authenticate_with_cognito_sync, username, password
    )
...
@v1_router.post("/login", tags=["Authentication"])
...
@v1_router.post("/token", tags=["Authentication"])
async def get_token(claims: dict[str, Any] = Depends(require_alb_authentication)):
    """
...
def welcome_page(claim=Depends(general_access_dependency)):
    ...
    html_content = html_content.replace("{username}", username).replace(
        "{fm_ui_url}", fm_ui_url
    )
