...
from lib.consts import CHANNELS, OUTPUT_DIR, TARGET_SOLAR_RADIUS, DOMAIN_NAME
...
class Infer:
    ...
    def _load_scalers(self):
        ...
        self._scaler_mean = np.array(
            [scalers[ch]["mean"] for ch in CHANNELS], dtype=np.float32
        ).reshape(n_channels, 1, 1)
        self._scaler_std = np.array(
            [scalers[ch]["std"] for ch in CHANNELS], dtype=np.float32
        ).reshape(n_channels, 1, 1)
        self._scaler_epsilon = np.array(
            [scalers[ch]["epsilon"] for ch in CHANNELS], dtype=np.float32
        ).reshape(n_channels, 1, 1)
        self._scaler_sl_factor = np.array(
            [scalers[ch]["sl_scale_factor"] for ch in CHANNELS], dtype=np.float32
        ).reshape(n_channels, 1, 1)
    ...
    def load_model(self):
        ...
        self.model = HelioSpectFormer(
            ...
            use_latitude_in_learned_flow=config["model"][
        ...
        # Compile model for optimized CUDA kernels
        # Use "default" mode to avoid CUDA graph conflicts with multiprocessing
        if torch.cuda.is_available():
    ...
    def apply_solar_disk_mask(data, mask, nodata_value=NODATA_VALUE):
        ...
        if masked_data.ndim == 3:
            # Vectorized: broadcast mask across all channels
    ...
    def load_netcdf(self, netcdf_path, transform=True):
        ...
        with xr.open_dataset(netcdf_path, engine="h5netcdf", chunks=None) as ds:
            ...
            timestamp = ds.attrs.get(
                "data_time",
                ds.attrs.get(
                    "timestamp_t_0", os.path.basename(netcdf_path).replace(".nc", "")
                ),
            )
    ...
    def compute_metrics(forecast, ground_truth, solar_mask=None):
        ...
        for i, channel in enumerate(CHANNELS):
            ...
            if len(pred) > 0:
                ...
                rel_error = (
                    np.abs(pred - true).mean() / (np.abs(true).mean() + 1e-8) * 100
                )

                metrics[channel] = {"correlation": corr, "rel_error": rel_error}
            else:
                metrics[channel] = {"correlation": 0.0, "rel_error": 999.9}

        return metrics

    def run_inference(
        self, first_step_file, second_step_file, steps=10, cadence_minutes=60
    ):
        """
        ...
        forecast_config = {"cadence_minutes": cadence_minutes, "max_steps": steps}
    ...
    def process_results(self, results, forecast_config):
        ...
        for result in results:
            ...
            correlation_entry = {"timestamp": timestamp_str, "score": score}
    ...
    def _extended_autoregressive_forecast(
        self, initial_input, forecast_config, start_time
    ):
        """
    ...
    def _cpu_autoregressive_forecast(
        self, initial_input, cadence_minutes, max_steps, start_time
    ):
    ...
    def _gpu_autoregressive_forecast(
        self, initial_input, cadence_minutes, max_steps, start_time
    ):
        """
        ...
        curr_batch = {
            ...
            "time_delta_input": torch.tensor([[-cadence_minutes, 0.0]]).to(
    ...
    def _save_single_geotiff(self, args):
        ...
        try:
            ...
            output_file = os.path.join(
                channel_dir, f"{timestamp}_{channel}_step{step_number:02d}.tif"
            )
            ...
            metadata.update(
                {
                    ...
                    "HORIZON_MIN": str(
                        step_number * forecast_config["cadence_minutes"]
                    ),
            ...
            dataset.SetGeoTransform(
                [origin_x, PIXEL_SCALE_ARCSEC, 0, origin_y, 0, -PIXEL_SCALE_ARCSEC]
            )
    ...
    def save_forecast_as_geotiff(
        self, forecast_data, timestamp, target_time, step_number, forecast_config
    ):
        """
        ...
        with ThreadPoolExecutor(max_workers=min(len(CHANNELS), 8)) as executor:
            futures = [
                executor.submit(self._save_single_geotiff, args) for args in save_args
            ]
